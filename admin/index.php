...
if (isset($_POST) && isset($_POST['update'])) {
    if ($csrfToken != $_POST['csrfToken']) {
        ...
        $ns->updatePage($_POST['type'], $_POST['slug'], $_POST['status'], $_POST['title'], $_POST['body'], $userID,
          $_POST['summary'], $_POST['sources'], null, null, null, null, $_POST['parent'], $_POST['showinnav'],
          $_POST['originalSlug']);
    }
    ...
    if ($csrfToken != $_POST['csrfToken']) {
        ...
        $ns->updatePlace($_POST['update-place'], $_POST['project'], $_POST['name'], $_POST['address'],
          $_POST['description']);
    }
    ...
    if ($_POST['action'] == 'addPage') {
        if ($csrfToken != $_POST['csrfToken']) {
            ...
            $ns->addPage($_POST['type'], $_POST['slug'], $_POST['status'], $_POST['title'], $_POST['body'], $userID,
              $_POST['summary'], $_POST['sources'], null, null, null, null, $_POST['parent'], $_POST['showinnav']);
        }
    ...
    if ($_POST['action'] == 'addPlace') {
        if ($csrfToken != $_POST['csrfToken']) {
            ...
            $ns->addPlace($_POST['project'], $_POST['name'], $_POST['address'], $_POST['imageUrl'],
              $_POST['description']);
        }
    }
...
if ($_POST['action'] == 'editUser') {
    ...
    } elseif (($_POST['self'] == false) && ($actualPrivilege >= $ns->getSettingValue('core',
          'adminPanelManageNonselfUser'))
    ) {
        $ns->updateUser($_POST['id'], $_POST['email'], $_POST['password'], $_POST['name'], $_POST['level'],
          $_POST['banned']);
    }
...
elseif(isset($_GET['deleteAllSessions']) && $_GET['deleteAllSessions'] == 'self'){
    if ($csrfToken != $_GET['csrfToken']) {
        die(header('Location: ../login.php'));
    } else {
        $ns->deleteAllSessions($userID);
        setcookie('ns_sid', 'loggedout', time() - 2700, null, null, null, true);
        setcookie('ns_sessionx', 'loggedout', time() - 2700, null, null, null, true);
        echo 'All sessions have been revoked. <a href="../login.php">Please log in again.</a>';
        die();
    }
