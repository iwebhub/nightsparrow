...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'logout') {
        $status = $ns->validateUserSession($_COOKIE['ns_sid'], $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT'],
          time());
        if ($status == true && $_GET['csrfToken'] == $ns->getSessionCSRF($_COOKIE['ns_sid'])) {
            $code = $ns->deleteSession($_COOKIE['ns_sid']);
        } else {
        ...
        if ($ns->emailExists($_POST['email']) == true) {
            ...
            if ($dv == true) {
                if ($_COOKIE['ns_csrf_anon'] == $_POST['csrfToken']) {
                    $sessionid = $ns->setUserSession($_POST['email'], $_SERVER['REMOTE_ADDR'],
