...
class Templates extends Nightsparrow {
  ...
  function scanForTemplates() {
    ...
    foreach ($scanresults as $result) { // and for each of the scanned items, do this:
      if ($result === '.' or $result === '..') continue; // if it's a . or .., just ignore it (why the f does php return this I just want to know about a situation where it was useful and not infuriating)
  ...
  function handleTemplateIncludes($file) {
    ...
    for ($i = 0; $i < $no; $i++) {
      ...
      $ns = new Nightsparrow();
      $file_to_include = rootdirpath . '/template/' . $ns->getSettingValue('core', 'siteActiveTheme') . '/' . $file_to_include . '.txt';
