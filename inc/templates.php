...
class Templates extends Nightsparrow {
  ...
  function adminGenerateTemplatePicker() {
    ...
    foreach ($templates as $template) {
      ...
      $type = $pcfParser->getPropertyValue($themefile, 'type');
      $license = $pcfParser->getPropertyValue($themefile, 'license');
      echo '<div class="single-theme s-u">
			<img src="'.$thumbpath.'" class="featured">
			<h2 class="key">'.$name.'</h2>
			<div class="info">';
      if($this->getSettingValue('core', 'siteActiveTheme') == $name){
      echo '<b>Trenutno aktivna :)</b>';
      }
      echo '<p>Autor: '.$author.'</p>
				<p>Opis: '.$description.'</p>
				<p>Licenca: '.$license.'</p>
			</div>
			<button class="activate-theme"><a href="?activate=' . $name . '">Aktiviraj</a></button>
		</div>';
  ...
  function handleTemplateIncludes($file) {
    ...
    for ($i = 0; $i < $no; $i++) {
      ...
      $ns = new Nightsparrow();
      $file_to_include = rootdirpath . '/template/' . $ns->getSettingValue(
          'core', 'siteActiveTheme'
        ) . '/' . $file_to_include . '.txt';
