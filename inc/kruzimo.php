...
class KruzimoHelper extends Nightsparrow {
  ...
  function getCodeAddressReferences($project) {
    ...
    while ($place = $res->fetch_assoc()) {
      ...
      $content = preg_replace('/[\x00-\x1F\x7F-\x9F]/u', '', $content); // redactor ubacuje 0A kontrolni znak za novi red iz nekog razloga, iako novi red nije potreban te taj novi red zbuni jadni javascript od gmapsa. uglavnom dva komada softvera u javascriptu se kolju. i onda mi netko predloži node.js kao programski jezik, fuj.
      ...
      $address = preg_replace('/[\x00-\x1F\x7F-\x9F]/u', '', $address); // redactor ubacuje 0A kontrolni znak za novi red iz nekog razloga, iako novi red nije potreban te taj novi red zbuni jadni javascript od gmapsa. uglavnom dva komada softvera u javascriptu se kolju. i onda mi netko predloži node.js kao programski jezik, fuj.
      $content .= '</p>';
