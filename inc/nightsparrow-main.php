...
class Nightsparrow {
    ...
    foreach ($structureMap as $row) {
      ...
      if ($row['parent'] == 0) {
        echo '<div class="pages-wrapper">
			<div class="single-wrapper s-u">

			<div class="dropdown">
				<div class="triangle"></div>
				<ul>
					<li><a href="add.php?update=' . $slug . '">Upravljanje stranicom</a></li>
					<li><a href="index.php?setAsHomepage=' . $slug . '&csrfToken=' . $csrfToken . '">Postavi kao početnu</a></li>
					<li><a href="add.php?parent=' . $slug . '">Stvori novu podstranicu</a></li>
				</ul>
			</div>';
        echo '<h2 class="key"><img src="../template/admin/img/page.png" class="thumb">';
        echo $title;
        echo '
					<span>
						<img src="../template/admin/img/'.$icons.'"><img class="dropbtn" src="../template/admin/img/more.png">
					</span>
				</h2>
				<span class="hidden key">';echo $title; echo'</span>
				<div class="info">';
					echo '<p>'.$summary.'</p>';
        ...
        if ($subpg > 3) {
          echo '<p><img src="../template/admin/img/story.png" class="post-thumb">Ova stranica ima mnogo podstranica.<a href="index.php?showSubstructureMap=' . $row['id'] . '">Prikaži sve</a></p>';
          echo '</div>
			</div>';
        }
        else {
        ...
        if ($caller == 'admin') {
          if ($subpg > 3) {
            echo '<p><img src="../template/admin/img/story.png" class="post-thumb">Ova stranica ima mnogo podstranica.  <a href="index.php?showSubstructureMap=' . $this->getPageAPI($row['slug'])['parent'] . '">Prikaži sve</a> </p>';          }
          else {
           echo '<p><img src="../template/admin/img/story.png" class="post-thumb">'.$title.' <span><a href="add.php?update=' . $slug . '"><img src = "../template/admin/img/edit.png"></a></span></p>';

            if ($this->countNumberOfChildPages($row['id']) > 0) {
              echo '<p><img src="../template/admin/img/story.png" class="post-thumb">Gornja podstranica ima podstranice.  <a href="index.php?showSubstructureMap=' . $row['id'] . '">Prikaži ih.</a></p>';
            }
          }
  ...
  function adminGetUserList() {
    ...
    foreach ($structureMap as $row) {
      ...
      echo '<div class="single-user s-u user-row">';
      ...
      if ($canEdit == true) {
        echo '<a href="user.php?id='.$row['id'].'"><span><img src="../template/admin/img/edit.png" class="nsic" alt=""></span></a>';
      }
			echo '</div>';
      $dbconn->close();
  ...
  function uploadFile($files) {
    $files['file']['name'] = substr(
        sha1($files['file']['name']), rand(0, 20), rand(0, 40)
      ) . '_' . strtolower($files['file']['name']);
  ...
  function sendEmail($email, $title, $template, $values) {
    ...
    $headers = "From: " . $this->getSettingValue(
        'core', 'siteName'
      ) . " (obavijesti)" . " <noreply.notifications@" . $domain . ">\r\n";
  ...
  function updateUser($id, $email, $password, $name, $level, $banned) {
    ...
    $time = $dbconn->real_escape_string(time());
    if (($level != null)
      && ($this->getUserPrivilege($this->getSessionUser($_COOKIE['ns_sid'])) != $this->getSettingValue(
          'core', 'adminPanelPermissionManageNonselfUser'
        ))
    ) {
