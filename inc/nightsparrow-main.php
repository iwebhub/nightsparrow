...
class Nightsparrow {
  ...
  /** DEPRECATED: funkcija za dodavanje korisnika, prima POST zahtjev sa podatcima, koristi se samo kod instalacije! u administraciji koristiti addUserAPI funkciju **/
  function addUser($post) {
    ...
    $currentTime = time();
    $addUserQuery = "INSERT INTO nb_users VALUES(null, '$email', '$password', '$uus', '$name', 10, 0, false, '$currentTime')";
...
/**
  ...
  function getPage($identifier, $theme) {
    ...
    $variables = array('%:pageTitle%%' => 'derp', '%:post.snip.url%%' => 'derp3', '%:homepage.intro%%' => $this->getSettingValue('layoutMessages', 'homepageIntro'));
  ...
  /** dodajmo stranicu :D **/
  function addPage($type, $slug, $status, $title, $body, $author, $summary, $sources, $mainimg, $password, $tags, $category, $parent, $showinnav) {
    $dbconn = $this->mysqliConnect();
    $type = $dbconn->real_escape_string($type);
    $slug = $dbconn->real_escape_string($slug);
  ...
  /** uredimo stranicu :D **/
  function updatePage($type, $slug, $status, $title, $body, $author, $summary, $sources, $mainimg, $password, $tags, $category, $parent, $showinnav, $originalSlug) {
    $dbconn = $this->mysqliConnect();
    $type = $dbconn->real_escape_string($type);
    $slug = $dbconn->real_escape_string($slug);
    $status = $dbconn->real_escape_string($status);
  ...
  /** dodajmo mjesto :D **/
  function addPlace($project, $name, $address, $imageUrl, $description) {
    $dbconn = $this->mysqliConnect();
    $project = $dbconn->real_escape_string($project);
    $name = $dbconn->real_escape_string($name);
    $address = $dbconn->real_escape_string($address);
    $imageUrl = $dbconn->real_escape_string($imageUrl);
    $description = $dbconn->real_escape_string($description);

    $time = $dbconn->real_escape_string(time());
    $sql = "INSERT INTO nb_places VALUES(null, '$project', '$name', '$description', '$address', '$imageUrl');";
    $res = $dbconn->query($sql);
    if ($res === false) {
      echo $dbconn->error;
      $this->throwError(0x010010);
    } else {
      echo '<div class="card-panel"><span class="blue-text text-darken-2"><i class="mdi-image-tag-faces"></i> Mjesto uspješno dodano</span></div>';
    }
    $dbconn->close();
  ...
  /** uredimo mjesto :D **/
  function updatePlace($id, $project, $name, $address, $description) {
    $dbconn = $this->mysqliConnect();
    $id = $dbconn->real_escape_string($id);
    $project = $dbconn->real_escape_string($project);
    $name = $dbconn->real_escape_string($name);
    $address = $dbconn->real_escape_string($address);
    $description = $dbconn->real_escape_string($description);
    $sql = "UPDATE nb_places SET project = '$project', name = '$name', address = '$address',  description = '$description' WHERE id = '$id'";
    $res = $dbconn->query($sql);
    if ($res === false) {
      echo $dbconn->error;
      $this->throwError(0x010010);
    } else {
      echo '<div class="card-panel"><span class="blue-text text-darken-2"><i class="mdi-image-tag-faces"></i> Mjesto uspješno uređeno</span></div>';
    }
    $dbconn->close();
  ...
  /** vraća popis mjesta na stranici, pozivati samo iz admin panela, namijenjeno za uređivanje **/
  function adminGetSitePlacesList() {
    $dbconn = $this->mysqliConnect();
    $res = $dbconn->query("SELECT * FROM nb_places ORDER BY id ASC") or die($dbconn->error . $this->throwError(0x010010));
    $structureMap = [];
    while ($row = $res->fetch_assoc()) {
      $structureMap[] = $row;
    }
    foreach ($structureMap as $row) {
      ...
      if ($row['project'] == $this->getSettingValue('kruzimo', 'project')) {
      ...
      <i class="' . $icons . ' circle"></i>
      <span class="title">' . $title . '</span>
      <p>' . $description . '<br> <b>Adresa: </b>' . $address;
      if (!$belongsToCurrent) {
        echo '<b> ne pripada trenutnom projektu</b>';
      }
      echo '</p><a href="addPlace.php?update=' . $row['id'] . '" class="secondary-content"><i class="mdi-editor-mode-edit"></i></a></li>';
    }
    $dbconn->close();
  ...
  function deleteSession($id, $sessionx, $ip, $useragent) {
    $dbconn = $this->mysqliConnect();
    $id = $dbconn->real_escape_string($id);
    $res = $dbconn->query("SELECT * FROM nb_sessions WHERE id = '$id' ORDER BY id ASC") or die($dbconn->error . $this->throwError(0x010010));
    $sessions = [];
    while ($row = $res->fetch_assoc()) {
      $sessions[] = $row;
    }
    foreach ($sessions as $session) {
      if ($session['cookieid'] == md5($sessionx . $ip . $useragent)) {
  ...
  function uploadPhoto($files) {
    ...
    ->uploadDir(rootdirpath . "uploads")->limitSize(["min" => 1000, "max" => 200000000])//limit image size (in bytes)
  ...
  function uploadFile($files) {
    $files['file']['name'] = substr(sha1($files['file']['name']), rand(0, 20), rand(0, 40)) . '_' . strtolower($files['file']['name']);
  ...
  function updateUser($id, $email, $password, $name, $level, $banned) {
    ...
    if(($level != null) && ($this->getUserPrivilege($this->getSessionUser($_COOKIE['ns_sid'])) != $this->getSettingValue('core', 'adminPanelPermissionManageNonselfUser'))){
