...
class PageGenerator extends Nightsparrow {
	function PageViewGenerator($pageID, $template) {
		...
		$pageTemplate = str_replace('%%header%%', file_get_contents(rootdirpath . 'template/' . $template . '/' . 'header.txt'), $pageTemplate);
		...
		$pageTemplate = str_replace('%%footer%%', file_get_contents(rootdirpath . 'template/' . $template . '/' . 'footer.txt'), $pageTemplate);
		...
		$pageTemplate = str_replace('{{siteDescription}}', $this->getSettingValue('core', 'siteDescription'), $pageTemplate);
		$pageTemplate = str_replace('{{design.headerContent}}', $this->getSettingValue('design', 'headerContent'), $pageTemplate);
		$pageTemplate = str_replace('{{design.footerContent}}', $this->getSettingValue('design', 'footerContent'), $pageTemplate);
        ...
        //$pageTemplate = str_replace('%%subnavigation%%', $this->SubnavigationGenerator($navbg, $template), $pageTemplate);
			...
			$blogObjectData = $data;
			$data = $this->getLatestChildrenPagesOfPageWithTypeAndStatusAPI($data['id'], $this->getSettingValue('generators', 'postsPerPage'), 'page', 0);
      //var_dump($data);

			$pageTemplate = str_replace('%%header%%', file_get_contents(rootdirpath . 'template/' . $temp . '/' . 'header.txt'), $pageTemplate);
			$pageTemplate = str_replace('{{pageTitleElement}}', $pageTitle, $pageTemplate);
			$pageTemplate = str_replace('%%navigation%%', $this->NavigationGenerator($pageID, $temp), $pageTemplate);
			$pageTemplate = str_replace('%%footer%%', file_get_contents(rootdirpath . 'template/' . $temp . '/' . 'footer.txt'), $pageTemplate);
			$pageTemplate = str_replace('{{domainpath}}', domainpath, $pageTemplate);
			$pageTemplate = str_replace('{{siteName}}', $this->getSettingValue('core', 'siteName'), $pageTemplate);
			$pageTemplate = str_replace('{{siteDescription}}', $this->getSettingValue('core', 'siteDescription'), $pageTemplate);
			...
			$postTemplate = substr($pageTemplate, strpos($pageTemplate, "[[post]]"), strpos($pageTemplate, "[[/post]]") - strpos($pageTemplate, "[[post]]"));
	...
	function NavigationGenerator($pageID, $template) {
		...
		if (strpos($itemTemplate, '((if active true))') != false) {
			...
			$pageItem = str_replace($this->tagParser($itemTemplate, "((if active true))", "((endif))"), "", $itemTemplate);
